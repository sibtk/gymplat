        ...
        {retentionImpact.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-2xl font-bold text-peec-dark">{item.value}</p>
      ...
      <div className="rounded-xl border border-peec-border-light bg-white p-5">
        ...
        <div className="grid grid-cols-1 gap-4 tablet:grid-cols-3">
          {riskBuckets.map((bucket) => (
            <div
              key={bucket.level}
              className="rounded-lg border border-peec-border-light/50 p-4"
            >
...
function CohortCell({ value }: { value: number }) {
  if (value === 0) {
    return <span className="text-2xs text-peec-text-muted">—</span>;
  }
